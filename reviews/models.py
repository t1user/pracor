...
class ApprovableModel(models.Model):
    ...
    approved = models.BooleanField("Zatwierdzone", default=None, null=True, blank=True)
...
class Company(ApprovableModel):
    ...
    employment = models.CharField(
        "zatrudnienie", max_length=1, choices=EMPLOYMENT, blank=True, null=True
    )
    public = models.BooleanField("notowane", blank=True, null=True)
    ...
    def reviews(self):
        return (
            Review.objects.selected(company=self.pk)
...
class Position(models.Model):
    ...
    company_linkedin_id = models.CharField(max_length=25, null=True, blank=True)
    ...
    end_date_month = models.PositiveIntegerField(
        null=True, blank=True, choices=MONTHS_E
    )
    end_date_year = models.PositiveIntegerField(choices=YEARS_E, default=0)
    employment_status = models.CharField(
        max_length=1, choices=STATUS_ZATRUDNIENIA, default="A"
    )
...
class Review(ApprovableModel):
    ...
    date = models.DateTimeField(
        "data", auto_now_add=True, editable=False, db_index=True
    )
    company = models.ForeignKey(
        Company, verbose_name="firma", on_delete=models.CASCADE, editable=False
    )
    position = models.OneToOneField(
        Position, verbose_name="stanowisko", on_delete=models.CASCADE, editable=False
    )
    ...
    overallscore = models.PositiveIntegerField(
        "ocena ogólna", choices=RATINGS, default=None
    )
    advancement = models.PositiveIntegerField(
        "możliwości rozwoju", choices=RATINGS, default=None
    )
    worklife = models.PositiveIntegerField(
        "równowaga praca/życie", choices=RATINGS, default=None
    )
    compensation = models.PositiveIntegerField("zarobki", choices=RATINGS, default=None)
    environment = models.PositiveIntegerField(
        "atmosfera w pracy", choices=RATINGS, default=None
    )
    ...
    def get_absolute_url(self):
        return reverse("company_page", kwargs={"pk": self.company.id})
...
class Salary(ApprovableModel):
    ...
    salary_gross_input_period = models.PositiveIntegerField(
        "pensja brutto", editable=False, default=0
    )
    salary_gross_annual = models.PositiveIntegerField(
        "pensja brutto rocznie", default=0, editable=False
    )
    bonus_gross_input_period = models.PositiveIntegerField(
        "premia brutto", null=True, editable=False
    )
    bonus_gross_annual = models.PositiveIntegerField(
        "premia brutto rocznie", null=True, editable=False
    )

    contract_type = models.CharField(
        "rodzaj umowy", max_length=1, default="A", choices=CONTRACT
    )
    ...
    def convert(self):
        ...
        def net_to_gross(N, zus=zus, ch=ch, z=z, ku=ku, kw=kw):
            ...
            # w przeciwnym wypadku użyj liczby podanej kwotowo w założeniach
            if ku < 1:
                B = (N - kw) / ((1 - zus - ch) * (1 - z - r_1 + 0.2 * r_1))
                ku = ku * B * (1 - zus - ch)
            else:
        ...
        self.salary_gross_input_period = round(
            self.salary_gross_annual / period_devisor[self.period], 0
        )
        ...
        if self.bonus_input:
            ...
            if self.bonus_gross_net == "N":
                ...
                self.bonus_gross_annual = (
                    total_comp_gross_annual - self.salary_gross_annual
                )
            else:
                self.bonus_gross_annual = bonus_annual

            self.bonus_gross_input_period = round(
                self.bonus_gross_annual / period_devisor[self.bonus_period], 0
            )
...
class Interview(ApprovableModel):
    ...
    how_got = models.CharField(
        "droga do interview", max_length=1, choices=HOW_GOT, default=None
    )
    difficulty = models.PositiveIntegerField(
        "trudność", choices=DIFFICULTY, default=None
    )
    got_offer = models.BooleanField(
        "dostał ofertę?",
    )
...
class AccessAttempt(models.Model):
    ...
    ip = models.GenericIPAddressField(
        protocol="both", unpack_ipv4=True, editable=False, blank=True, null=True
    )
